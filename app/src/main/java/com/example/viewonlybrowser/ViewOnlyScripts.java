...
final class ViewOnlyScripts {
    ...
    static String targetDetector(String detectedUrl, boolean readonlyEnabled, boolean functionBlockingEnabled) {
        return "(function(){"
                ...
                + "function hasAccountList(){return !!document.querySelector("
                + "'#page_container tr.account_item[account_id] a[href^=\"javascript:load_account(\"]');}"
                // Stable page chrome only. No customer, account, or balance text is inspected.
                + "function hasDashboardChrome(){return hasText('.logout','se déconnecter')"
                + "&&hasText('lng[key=\"newShareApplication.welcome\"]','bienvenue');}"
                + "function isTarget(){return hasDashboardChrome()&&(hasAccountList()||"
                + "hasText('lng[key=\"accounts.depositAccounts\"]','comptes de dépôt'));}"
                + "function signal(){if(window.SogeMobileBridge&&typeof "
                + "window.SogeMobileBridge.onDashboardDetected==='function'){"
                + "window.SogeMobileBridge.onDashboardDetected();return;}"
